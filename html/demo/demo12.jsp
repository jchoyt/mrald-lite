...
<option value="Table:PLAYERS~Field:NAMELAST~Order:1" selected="">Last Name</option>
...
<p class="helpBoxTitle">Tutorial - Slide 13 : Output Formatting Options (cont.)</p>
<p class="helpBoxText">Another option is whether duplicate data is to be returned or not. </p>
<p class="helpBoxText">Sometimes, due to the query, or due to the way data is stored in the database, the same results will appear on multiple rows.</p>
<p class="helpBoxText">To see an example of this, select the <i>Duplicate Rows</i> option and click on Retrieve Data, and you will see that there are multiple instances of the same data.</p>
<p class="helpBoxText">This is because there is a salary (and one row of data) for each player for each year - but the year is not selected in the output options, so the data is repeated.</p>
<p class="helpBoxText">Now deselect the <i>Duplicate Rows</i> option, and repeat. Now only a single line appers for each row that was previously duplicated</p>
<center><p class="helpBoxText"><button type="button" onclick="location='demo11.jsp'"><b>Previous Slide</b></button><button type="button" onclick="location='demo13.jsp'"><b>Next Slide</b></button></p></center>
...
<input name="Filter2" type="text" size="22" value="2001"/>
<br/>
<select name="Filter3">
<option/>
