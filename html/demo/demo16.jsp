...
<br/>
<input name="Select9" type="checkbox" value="Table:TEAMS~Field:TEAMID~Order:2" onMouseOver="if (EnableStats.value == 1) {}" onFocus="if (EnableStats.value == 1) {this.click(), this.blur()}" onDblClick="if (EnableStats.value == 1) {this.click(), this.blur()}" checked="">Team ID</input>
<br/>
<br/>
...
<p class="helpBoxTitle">Tutorial - Slide 17 : Destination</p>
<p class="helpBoxText">So far all results have been displayed through a browser.</p>
<p class="helpBoxText">In addition to the browser format the data can be output to a file, or spreadsheet application.</p>
<p class="helpBoxText">The file format allows for data to be comma, <i>CSV</i> or tab delimited.</p>
<p class="helpBoxText">Select the <i>CSV</i> format, and on clicking Retrieve Data button, you will be presented with a 'Save As' dialogue window.	After saving the file to your file system, on opening this file you will see the same data as appeared in the browser.
<p class="helpBoxText">The <i>Excel</i> format allows the user to save the data to a spreadsheet format, to allow for further analysis or charting of data.</p>
<p class="helpBoxText">File formats allow for data to be input into other systems, via a file. Or for large data sets to be downloaded for viewing later.</p>
<center><p class="helpBoxText"><button type="button" onclick="location='demo15.jsp'"><b>Previous Slide</b></button><button type="button" onclick="location='<%=Config.getProperty("BaseUrl")%>/index.jsp'"><b>Home</b></button></p></center>
</div>
...
<input name="Filter2" type="text" size="22" value="2004"/>
<br/>
<select name="Filter3">
<option/>
